...
class ModelSummary:
    ...
    def __call__(self):
        ...
        summary_str = self._create_summary_string(summary_dict, total_params, trainable_params, total_macs, total_flops, table)
    ...
    def _create_summary_dict(self):
        ...
        x = [torch.rand(2, *in_size).type(dtype).to(device=self.device)
    ...
    def _create_table(self, summary_dict):
        ...
        table.add_column("Layer (type)", justify="right", style="light_cyan3", no_wrap=True)
    ...
    def _create_summary_string(self, summary_dict, total_params, trainable_params, total_macs, total_flops, table):
        ...
        """
        total_output = sum(np.prod(summary_dict[layer]["output_shape"]) for layer in summary_dict)
        # assume 4 bytes/number (float on cuda).
        total_input_size = abs(np.prod(sum(self.input_size, ()))
                               * self.batch_size * 4. / (1024 ** 2.))
        ...
        summary_str += "-" * (len(str(table).splitlines()[0]) - 3) + "\n"  # Dynamic line
    ...
    def _table_to_df(self, rich_table: Table, remove_markup: bool = True) -> pl.DataFrame:
    ...
    def _calculate_macs(self, module, output):
        ...
        if isinstance(module, nn.Conv2d):
            ...
            macs = module.kernel_size[0] * module.kernel_size[1] * module.in_channels * module.out_channels * output_size[2] * output_size[3]
