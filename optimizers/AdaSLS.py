...
class AdaSLS(torch.optim.Optimizer):
    """
    Implements AdaSLS
    Source: https://github.com/IssamLaradji/ada_sls/blob/master/adasls/adasls.py
    Paper: Adaptive Gradient Methods Converge Faster with Over-Parameterization (but you should do a line-search)
    PDF: https://arxiv.org/abs/2006.06835
    """
    def __init__(self,
                 params,
    ...
    def get_step_size(self, closure_deterministic, loss, params_current, grad_current, grad_norm, pp_norm,
        ...
        if self.step_size_method == 'sps':
            ...
            # reset step size
            # step_size=self.state['step_size']
            step_size = reset_step(step_size=self.state['step_size'],
